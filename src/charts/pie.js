  ...
  render: function() {
    ...
    var slices = chart.curves.map(function(c, i) {
      return (
        <g key={ i } transform={ self.move(c.sector.centroid, coefficients[i]) }>
          ...
          <text textAnchor="middle" transform={ self.translate(c.sector.centroid) }>{ c.item.mi }</text>
        </g>
      )
    });
    ...
    var table = selected ?
      <div className="country-info">
        <h4>Mitter Id : { selected.mi }</h4>
        <p>Used Litter: <span className="label label-info">{ selected.used_litter }</span></p>
      </div> : null

    return(
      <div id="pie" style={header}>
      <div>  <svg width="875" height="400">
          <g transform="translate(500, 200)">{ slices }</g>
        </svg>
</div>
        <div>{ table }</div>
      </div>
